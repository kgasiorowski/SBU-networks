...
if len(sys.argv) == 1:
    print('No port passed, using default {0}'.format(DEFAULT_PORT))
    PORT = DEFAULT_PORT
else:
    try:
        PORT = int(sys.argv[1])
    except:
        print('Error: invalid port passed in. Using default of {0}'.format(DEFAULT_PORT))
        PORT = DEFAULT_PORT
...
if __name__ == '__main__':
    ...
    try:
        ...
        print("Error - the selected port was denied by the system. Continuing with default of {0}".format(DEFAULT_PORT))
